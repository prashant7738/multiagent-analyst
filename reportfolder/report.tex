...
Prashant Kafle (THA080BCT026) & \rule{5cm}{0.4pt} \\[1.2em]
Roshan Poudel (THA080BCT036) & \rule{5cm}{0.4pt} \\[1.2em]
Santosh Khadka (THA080BCT039) & \rule{5cm}{0.4pt} \\[1.2em]
Sushil Bhatta (THA080BCT048) & \rule{5cm}{0.4pt} \\
\end{tabular}
...
\texttt{/api/jobs/\{id\}} & DELETE & Delete a job and its artifacts \\
\texttt{/api/jobs} & DELETE & Delete all jobs \\
\texttt{/api/plots/\{file\_path\}} & GET & Serve a static chart PNG, isolated per job \\
\texttt{/api/health/test-llm} & POST & Manual provider health test, bypasses the health-check cache \\
\hline
\end{tabular}
\end{table}
...
\end{equation}
where $m$ is the overall missing rate and $d$ the duplicate rate as percentages. The stage is marked \texttt{ready} when $c_1 \geq 0.8$ and \texttt{needs\_review} otherwise. This 0.8 cutoff applies only to Agent~1's stage-level confidence; it is separate from the pipeline's overall decision-readiness threshold of 0.85 introduced in Chapter~5. Each stage sets its own readiness cutoff before the pipeline averages the stage confidences into a single overall score. On success it writes \texttt{raw\_profile}, caches the DataFrame in \texttt{\_df\_cache}, and passes both to Agent~2; if loading fails, the error is recorded and the pipeline stops early. Tested on the \texttt{10000 Sales Records.csv} dataset, Agent~1 parsed the file without row loss and produced the per-column missingness and duplicate counts consumed by the later agents.
...
\subsection{LLM Provider Reliability}
The system calls Groq first and falls back to Gemini when Groq is unavailable or its quota is exhausted. Free-tier Gemini quotas run out quickly, so Agent~2 and the chat service accept several keys through \texttt{GEMINI\_API\_KEYS} (a comma, space, or semicolon-separated list) or the numbered variables \texttt{GEMINI\_API\_KEY\_1} through \texttt{GEMINI\_API\_KEY\_5}. When a key returns a quota error, the caller rotates to the next key immediately instead of retrying with backoff, which keeps the pipeline from stalling on repeated 429 responses. Model selection follows the same pattern: the system tries \texttt{gemini-flash-latest} first, then \texttt{gemini-3.6-flash}, \texttt{gemini-3.5-flash}, and \texttt{gemini-2.5-flash} in order, so a retired or renamed model does not stop the pipeline. Health checks for Groq, Gemini, Hugging Face, and PostgreSQL are cached for 90 seconds to avoid spending quota on passive polling; \texttt{POST /api/health/test-llm} bypasses that cache when an on-demand connectivity test is needed.

\subsection{RAG Dataset Chat}
The RAG service uses \texttt{BAAI/bge-base-en-v1.5} embeddings via the Hugging Face Inference API when the required \texttt{DATABASE\_URL} and Hugging Face token are configured. pgvector stores embeddings for row documents from the original uploaded CSV and fact documents derived from the generated analysis context. During retrieval, the service fetches top row documents (default $K=8$), top column/statistic fact documents (default $K=6$), and always includes dataset-wide facts such as summaries, validation results, correlations, rankings, anomalies, and existing charts. The chat service passes the retrieved context and recent chat history to Groq $\rightarrow$ Gemini $\rightarrow$ deterministic fallback for response generation. If the database or embedding service is unavailable, the fallback answers from deterministic summary facts rather than claiming vector retrieval. Users can also ask for a new chart in plain language within the same chat. \texttt{render\_chart\_request} checks the request against a fixed whitelist of chart types (bar, line, histogram, box, scatter) and, for ad-hoc numeric queries, of aggregation operations (count, sum, mean, median, min, max, nunique). A request outside that whitelist is rejected rather than executed.
...
\subsection{Comparison with a General-Purpose Qwen LLM}

The same automated insight-generation task was also evaluated against a general-purpose Qwen LLM using the \texttt{10000\_Sales\_Records.csv} dataset. The comparison shows the practical value of separating deterministic analysis from language generation. The multi-agent pipeline completed in approximately 52 seconds, while the standalone Qwen run took approximately 4 minutes 12 seconds, making it about 4.8 times slower.

\begin{table}[H]
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
\caption{Comparison of the multi-agent pipeline and a general-purpose Qwen LLM}
...
Actionability & Findings were tied to specific evidence, such as Snacks averaging 26.1 shipping days versus 24.7 days for Fruits & Recommendations were broader and less tied to specific evidence \\
\hline
\end{tabular}
\end{table}

The multi-agent pipeline was therefore approximately five times faster and was the only system in this comparison whose output was independently verifiable against the source data. The general-purpose LLM produced directionally useful results, but its consistent 10 times scaling error shows why a validation layer matters. Output Validation does more than add a reporting step. It stops numerical errors of this kind from reaching the user unnoticed.
...
The implementation has functional tests for RAG triggering, context construction, retrieval branches, fallback behavior, and validated on-demand chart generation. The K=8 retrieval results are shown in Tables \ref{tab:rag-k8-summary} and \ref{tab:rag-k8-per-query}.

\begin{table}[H]
\centering
\caption{RAG retrieval results at K=8}
\label{tab:rag-k8-summary}
...
\begin{table}[H]
\centering
\caption{Per-query RAG retrieval results at K=8}
\label{tab:rag-k8-per-query}
\begin{tabular}{p{7.2cm} p{1.5cm} p{1.5cm} p{1.5cm} p{1.5cm}}
\hline
Query & Precision & Recall & nDCG & RR \\
\hline
Orders from Europe & 1.000 & 0.0030 & 1.000 & 1.000 \\
Orders from Sub-Saharan Africa & 1.000 & 0.0031 & 1.000 & 1.000 \\
Household item orders & 1.000 & 0.0091 & 1.000 & 1.000 \\
Cosmetics item orders & 1.000 & 0.0096 & 1.000 & 1.000 \\
Orders through the Online sales channel & 1.000 & 0.0016 & 1.000 & 1.000 \\
Orders with High priority & 0.500 & 0.0016 & 0.601 & 1.000 \\
Orders with unit price above 500 & 0.750 & 0.0035 & 0.793 & 1.000 \\
Orders with units sold above 8000 & 0.000 & 0.0000 & 0.000 & 0.000 \\
Orders with total profit above 1000000 & 0.000 & 0.0000 & 0.000 & 0.000 \\
Household item orders from Europe & 0.500 & 0.0190 & 0.582 & 1.000 \\
High priority orders from Asia & 0.375 & 0.0082 & 0.497 & 1.000 \\
Offline Vegetables orders with units sold above 7000 & 0.000 & 0.0000 & 0.000 & 0.000 \\
\hline
\end{tabular}
\end{table}
