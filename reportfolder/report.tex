...
The LLM backend of this system is the Groq API, which exposes the \texttt{llama-3.3-70b-versatile} model through an OpenAI-compatible REST interface. Groq achieves exceptionally low inference latency by running Transformer models on Language Processing Units (LPUs), custom silicon designed specifically for autoregressive token generation. This section explains the Groq inference model and how Agent~2 uses it to generate the JSON schema blueprint.

\subsection{Why the Groq API Is Used}
The Groq API was selected over local inference runtimes for three principal reasons.

\textbf{Inference speed.} LPU hardware is architected for the sequential, memory-bound nature of autoregressive decoding. Each forward pass generates one token, and the dominant cost is reading model weights from memory on every step. LPUs use on-chip SRAM instead of off-chip DRAM, eliminating memory bandwidth as the bottleneck. In practice, \texttt{llama-3.3-70b-versatile} on Groq produces hundreds of output tokens per second---far more than a CPU or consumer GPU running the same model weights locally. For a pipeline where Agent~2 may need to tag dozens of columns in one call, short round-trip times keep the overall pipeline latency acceptable.

\textbf{No local hardware requirement.} Running a 70-billion-parameter model locally demands tens of gigabytes of GPU VRAM. The Groq API allows the project to use a state-of-the-art open-weight model without imposing any GPU requirement on the developer's machine. This makes the system deployable on standard laptops and cloud VMs alike.
...
\subsection{The Llama 3.3 70B Model}
\texttt{llama-3.3-70b-versatile} is a 70-billion-parameter decoder-only Transformer released by Meta as part of the Llama~3 family \cite{dubey2024llama3}. Its architecture follows the design described in Section~4.1 with several refinements that improve efficiency and long-context handling.
...
\textbf{RoPE positional encoding.} Instead of adding a fixed positional vector to each embedding, Llama~3 applies Rotary Position Embedding (RoPE), which encodes absolute position by rotating the query and key vectors before the dot product:
\begin{equation}
...
\textbf{SwiGLU feed-forward blocks.} Each Transformer block contains a position-wise feed-forward network. Llama~3 replaces the standard ReLU activation with SwiGLU:
\begin{equation}
...
\textbf{RMS Layer Normalization.} Llama~3 uses Root Mean Square Layer Normalization (RMSNorm) instead of the standard LayerNorm:
\begin{equation}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth,height=0.82\textheight,keepaspectratio]{llm_transformer_architecture.png}
\caption{Llama~3.3-70B architecture: decoder-only Transformer with GQA, RoPE, SwiGLU, and RMSNorm}
...
\begin{enumerate}
  ...
  \item \textbf{API request.} The Groq Python client sends a \texttt{chat.completions.create} request with \texttt{model="llama-3.3-70b-versatile"} and \texttt{temperature=0.1}. A temperature close to zero makes the softmax distribution (Equation~\ref{eq:softmax}) sharply peaked, so the model consistently selects the highest-probability token rather than sampling freely. This near-determinism is important because the output must be strictly parseable JSON.
  \item \textbf{Prompt processing.} On Groq's LPU cluster the prompt tokens are tokenized and embedded (Equation~\ref{eq:embed}), then passed through the 80 decoder blocks of Llama~3.3-70B. Inside each block, GQA (Equation~\ref{eq:gqa}) computes position-aware attention using RoPE (Equation~\ref{eq:rope}), and the SwiGLU FFN (Equation~\ref{eq:swiglu}) applies non-linear gating. RMSNorm (Equation~\ref{eq:rmsnorm}) stabilizes each sub-layer. The final block projects to logits over the vocabulary and applies softmax to produce the next-token distribution.
...
This design keeps Agent~2 loosely coupled to the model provider: the entire LLM interaction is isolated inside three functions (\texttt{\_build\_llm\_prompt}, the Groq client call, and \texttt{\_parse\_schema\_blueprint\_response}), so the backend can be swapped for another OpenAI-compatible endpoint without touching the rest of the pipeline.
...
\section{Methodology}
The methodology combines deterministic data processing with LLM-assisted semantic reasoning. Structural checks, preprocessing rules, and statistics are implemented in Python, while the validation and reporting stages remain planned work. Semantic interpretation uses the Groq API with the \texttt{llama-3.3-70b-versatile} model, and the planned final narrative generation will use the same backend; both stages are backed by fallback logic so the pipeline remains usable when the API is unavailable.
...
\bibitem{dubey2024llama3} A. Dubey et al., ``The Llama 3 Herd of Models,'' arXiv preprint arXiv:2407.21783, 2024.
